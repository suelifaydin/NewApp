    ...
    <ImageBackground source={backgroundImage} style={styles.container} key={theme}>
      ...
      <ScrollView contentContainerStyle={{ paddingBottom: 40 }}>
        <View style={styles.infoBox}>
          <Text style={styles.title}>{t('nightIsland')}</Text>
          <Text style={styles.subTitle}>45 MIN • {t('sleepMusic')}</Text>
          <Text style={styles.description}>{t('sleepDesc')}</Text>
        ...
        {/* Favori parçalar varsa göster */}
        {profileStore.favorites.length > 0 && (
          <View style={styles.relatedSection}>
            <View style={styles.relatedHeader}>
              <Text style={styles.relatedTitle}>{t('yourFavorites')}</Text>
